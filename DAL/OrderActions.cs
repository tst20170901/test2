...
namespace DAL
{
    ...
    {
        public static int Add(Models.OrderActions model)
        {
            try
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append("insert into [OrderActions] ([Oid],[Uid],[RealName],[ActionType],[ActionDes]) values (@Oid,@Uid,@RealName,@ActionType,@ActionDes);");
        ...
        public static List<Models.OrderActions> GetListByOrderID(int orderID)
        {
            try
            {
                string sql = "select * from [UserOrderActions] where [Oid]=@Oid order by [ActionTime] desc";
                ...
                List<Models.OrderActions> list = new List<Models.OrderActions>();
                if (dt != null && dt.Rows.Count > 0)
                {
                    foreach (DataRow item in dt.Rows)
                    {
                        Models.OrderActions model = new Models.OrderActions();
